...
correlation = merged_df.select([
    pl.corr("BTC_LogRatio", "ETH_LogRatio").alias("Correlation")
])
...
plt.axvline(btc_mean_return, color='blue', linestyle='dashed', linewidth=2, label=f'BTC Mean: {btc_mean_return:.4f}')
plt.axvline(eth_mean_return, color='orange', linestyle='dashed', linewidth=2, label=f'ETH Mean: {eth_mean_return:.4f}')
...
log_ratio_correlation = merged_df.select([
    pl.corr("BTC_LogRatio", "ETH_LogRatio").alias("Log_Ratio_Correlation")
])
...
plt.text(0.05, 0.95, f"Correlation: {correlation_value:.4f}", transform=plt.gca().transAxes,
...
try:
    ...
    portfolio_value = (CURRENT_PORTFOLIO['BTC'] * current_btc_price) + (CURRENT_PORTFOLIO['ETH'] * current_eth_price)
...
btc_value = CURRENT_PORTFOLIO['BTC'] * current_btc_price
eth_value = CURRENT_PORTFOLIO['ETH'] * current_eth_price
total_value = btc_value + eth_value

print("\nCurrent Portfolio Breakdown:")
print(f"BTC: {CURRENT_PORTFOLIO['BTC']} BTC = ${btc_value:.2f}")
print(f"ETH: {CURRENT_PORTFOLIO['ETH']} ETH = ${eth_value:.2f}")
...
# Calculate and print some statistics about the portfolio values
portfolio_stats = portfolio_values_series.describe()
print("\nPortfolio Value Statistics:")
print(portfolio_stats)

# Calculate and print the 5th percentile of portfolio values (95% VaR)
var_95 = portfolio_values_series.quantile(0.05)
var_95_percentage = ((total_value - var_95) / total_value) * 100
print(f"\n95% VaR: ${total_value - var_95:.2f} ({var_95_percentage:.2f}% of current portfolio value)")

# Plot the distribution of portfolio values
plt.figure(figsize=(10, 6))
sns.histplot(portfolio_values_series, kde=True)
plt.title("Distribution of Simulated Portfolio Values")
plt.xlabel("Portfolio Value ($)")
plt.ylabel("Frequency")
plt.axvline(total_value, color='r', linestyle='--', label='Current Value')
plt.axvline(var_95, color='g', linestyle='--', label='95% VaR')
plt.legend()
plt.show()
